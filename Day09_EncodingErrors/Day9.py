    ...
    for x in range(len(preamble)):
        for y in range(len(preamble)):
            ...
            if ((valueX+valueY)==int(valueToGet) and (valueX!=valueY)):
                print(f"Found a solution with the sum of {valueX} and {valueY}")
                return True
...
def question_two_recursive(pointer_high_boundery, current_list,current_sum):
    ...
    if current_list == []:
        ...
        question_two_recursive(pointer_high_boundery, current_list,current_sum)
...
#print(question_two_recursive(0, [],0)) #Fails with: RecursionError: maximum recursion depth exceeded in comparison
...
def question_two_iterative_helper():
    ...
    while solution_found == False: 
        pointer_high_boundery, current_list,current_sum, solution_found= question_two_iterative(pointer_high_boundery, current_list,current_sum,solution_found)
